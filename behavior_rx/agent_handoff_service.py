...
class AgentHandoffService:
    ...
    async def _persist_handoff(
        ...
        """持久化交接记录 (兼容 sync/async DB session)"""
        try:
            ...
            if hasattr(db, 'flush') and not _is_coroutine_function(db.flush):
                db.flush()
                db.commit()
            else:
                await db.flush()
    ...
    async def _update_handoff_status(
        ...
        """更新交接状态 (兼容 sync/async DB session)"""
        try:
            from .rx_models import AgentHandoffLog
            ...
            if hasattr(db, 'execute') and not _is_coroutine_function(db.execute):
                db.execute(stmt)
                db.flush()
                db.commit()
            else:

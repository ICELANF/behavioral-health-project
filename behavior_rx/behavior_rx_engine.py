...
class BehaviorRxEngine:
    ...
    def get_strategy_templates(
        self, domain: Optional[str] = None, enabled_only: bool = True
    ) -> list:
        """返回策略模板列表 (供 REST API 使用)"""
        templates = list(self._strategy_templates.values())
        if domain:
            templates = [t for t in templates if t.get("domain", "general") == domain]
        if enabled_only:
            templates = [t for t in templates if t.get("is_enabled", True)]
        return templates
    ...
    async def _persist_rx(self, rx: RxPrescriptionDTO, context: RxContext, db) -> None:
        ...
        try:
            ...
            if asyncio.iscoroutinefunction(db.flush) or inspect.iscoroutinefunction(db.flush):
                await db.flush()
                await db.commit()
            else:
                db.flush()
            ...
            logger.error(f"Failed to persist RxPrescription: {e}", exc_info=True)

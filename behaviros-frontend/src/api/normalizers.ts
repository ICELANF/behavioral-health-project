...
function normalizeStage(raw: string | undefined | null): JourneyStage {
  if (!raw) return JourneyStage.S0_AUTHORIZATION
  // 已经是全称
  if (raw.startsWith('s0_') || raw.startsWith('s1_') || raw.startsWith('s2_') ||
      raw.startsWith('s3_') || raw.startsWith('s4_') || raw.startsWith('s5_')) {
    return raw as JourneyStage
  }
  // 短码 "S0" → "s0_authorization"
  return (STAGE_SHORT_TO_FULL[raw.toUpperCase()] || raw) as JourneyStage
}

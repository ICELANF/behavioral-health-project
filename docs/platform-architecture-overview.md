...
│  │ 77 页面        │  │  18 页面       │  │  专家督导     │  │  10 页面       │  │
│  │ 5 stores       │  │  2 stores      │  │              │  │  4 stores      │  │
│  └───────┬────────┘  └───────┬────────┘  └──────┬───────┘  └────────────────┘  │
│          │                   │                   │                              │
│          └──────────┬────────┘                   │                              │
│                     │ nginx反代 /api → bhp-api   │                              │
...
| 模块 | 前缀 | 端点数 | 功能 |
|------|------|--------|------|
| `auth_api.py` | `/api/v1/auth` | 6 | 登录/注册/刷新/改密/登出/个人信息 |
| `user_api.py` | `/api/v1/admin` | 11 | 用户CRUD/统计/分配/教练列表 |
| `coach_api.py` | `/api/v1/coach` | 12 | 学员看板/绩效/画像/4类健康数据 |
| `coach_message_api.py` | `/api/v1/coach/messages` | 6 | 教练↔学员消息/收件箱/已读 |
| `coach_push_queue_api.py` | `/api/v1/coach/push-queue` | 7 | 推送审批队列/统计/批量操作 |
| `micro_action_api.py` | `/api/v1/micro-actions` | 6 | 今日任务/完成/跳过/历史/统计 |
| `reminder_api.py` | `/api/v1/reminders` | 5 | 提醒CRUD/教练代建 |
| `challenge_api.py` | `/api/v1/challenges` | 28 | 挑战模板CRUD/审核/报名/打卡/问卷/教练分配 |
| `assessment_api.py` | `/api/assessment` | 5 | 评估提交/历史/详情 |
| `assessment_pipeline_api.py` | `/api/v1/assessment` | 4 | BAPS→画像→阶段→干预全流程 |
| `assessment_assignment_api.py` | `/api/v1/assessment-assignments` | 9 | 教练分配/审核/推送评估 |
| `device_rest_api.py` | `/api/v1/devices` | 11 | 设备绑定/健康数据 |
| `device_data.py` | `/api/v1/mp/device` | 23 | 血糖/体重/血压/睡眠/运动/HR/HRV |
...
| `content_api.py` | `/api/v1/content` | 24 | 课程/视频/测验/案例/学习进度 |
| `learning_api.py` | `/api/v1/learning` | 14 | 积分/时长/连续打卡/奖励/排行榜 |
...
| `paths_api.py` | `/api/v1/paths` | 46 | 六大路径（干预/专家/知识/实践/社区/教练） |
| `segments_api.py` | `/api/v1/segments` | 11 | 权限/功能/角色/层级/升级建议 |
| `miniprogram.py` | `/api/v1/mp` | 18 | 小程序端（任务/反馈/对话/进度/风险） |
| `websocket_api.py` | WebSocket | 1 | 实时推送 |
| `baps_api.py` | — | 3 | 问卷/评分 |
| `routes.py` | `/api/v1` | 6 | 通用路由 |

**总计: 294+ 端点** (另有 v14 遗留路由 50 端点)
...
| `scheduler.py` | APScheduler 5任务调度 |
...
| 角色 | 入口路由 | 核心功能 |
|------|---------|---------|
| **管理员** | `/admin/*` | 用户管理、分配、挑战管理、Prompt管理、干预包、系统设置 |
| **教练** | `/coach-portal` | 学员看板(Kanban)、7个干预工具、评估/目标/推送抽屉、挑战分配、推送队列审批、全平台搜索 |
| **专家** | `/expert-portal` | 督导看板、案例审核、教练评估、研究数据 |
...
**18 个页面 · 2 个 Store · 5 个 API 模块**

```
路由 (19条):
├── /login                  登录（含快速体验按钮）
├── /                       首页（微行动卡片+连续打卡+挑战卡片+预警横幅）
├── /chat                   AI对话
├── /tasks                  微行动任务管理
├── /dashboard              健康数据看板
├── /profile                个人资料
...
| 指标 | 数值 |
|------|------|
| SQLAlchemy 数据模型 | 33 个 |
| 数据库迁移版本 | 8 个 |
| API 路由模块 | 27 个 (+ v14 遗留 5个) |
| API 端点总数 | 294+ (+ v14 遗留 50) |
| 核心服务文件 | 31 个 |
| 定时任务 | 5 个 |
| Docker 容器 | 13 个 (4 应用 + 9 基础设施) |
| Admin Portal 页面 | 77 个 .vue |
| H5 移动端页面 | 18 个 .vue |
| H5-Patient-App 页面 | 10 个 .vue |
| Pinia Store | 11 个 (Admin 5 + H5 2 + Patient 4) |
| 专业 Agent | 12 个 |
...
| 模块 | 状态 |
|------|------|
| 全部 33 个数据模型 + 8 版迁移 | ✅ 数据库就绪 |
| 27 个 API 路由模块 (294+ 端点) | ✅ 全部注册 |
| JWT 认证 + 7 级 RBAC | ✅ 运行中 |
| APScheduler 5 个定时任务 | ✅ 已启动 |
| 12 个专业 Agent + MasterAgent | ✅ 在线 |
| 推送审批网关 | ✅ 运行中 |
| Docker 4 应用容器 | ✅ healthy |
| Admin Portal 全平台搜索 | ✅ 运行中 |
| H5 快速体验登录 | ✅ 运行中 |
| Dify + Ollama LLM | ✅ 运行中 |

<template>
  <div>
    <div class="page-header"><h2>晋级复核（L2）</h2></div>
    <div class="card">
      <div v-if="loading" class="loading">加载中...</div>
      <div v-else-if="items.length === 0" class="empty">暂无待复核晋级申请</div>
      <table v-else class="data-table">
        <thead>
          <tr><th>申请人</th><th>当前等级</th><th>目标等级</th><th>L1审核人</th><th>时间</th><th>状态</th><th>操作</th></tr>
        </thead>
        <tbody>
          <tr v-for="item in items" :key="item.id">
            ...
            <td><span class="status-tag" :class="'st-' + item.status">{{ item.status }}</span></td>
            <td class="action-cell">
              <button v-if="item.status === 'l1_approved'" class="action-btn action-approve" @click="approve(item)">复核通过</button>
              <button v-if="item.status === 'l1_approved'" class="action-btn action-reject" @click="reject(item)">驳回</button>
            </td>
          </tr>
        </tbody>
      </table>
...
async function load() {
  loading.value = true
  try {
    const res: any = await api.get('/api/v1/promotion/applications?stage=L2')
...
.st-l1_approved { background: #dbeafe; color: #1d4ed8; }
.st-l2_approved { background: #d1fae5; color: #065f46; }

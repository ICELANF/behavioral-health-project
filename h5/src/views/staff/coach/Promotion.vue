<template>
  <div>
    ...
    <div class="card">
      ...
      <div v-else-if="items.length === 0" class="empty">暂无待审晋级申请</div>
      <table v-else class="data-table">
        <thead>
          <tr><th>申请人</th><th>当前等级</th><th>目标等级</th><th>申请时间</th><th>状态</th><th>操作</th></tr>
        </thead>
        <tbody>
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="action-cell">
              <button v-if="item.status === 'pending'" class="action-btn action-approve" @click="approve(item)">通过</button>
...
async function load() {
  loading.value = true
  try {
    const res: any = await api.get('/api/v1/promotion/applications?stage=L1')
    items.value = res.items || (Array.isArray(res) ? res : [])
  } catch {
    try {
      const res: any = await api.get('/api/v1/journey/promotion/history')
      items.value = res.items || (Array.isArray(res) ? res : [])
    } catch { items.value = [] }
  }
  loading.value = false

...
RED_FLAG_PATTERNS: list[tuple[str, str]] = [
    # (正则模式, 规则描述)
    (r"剧烈头痛.{0,50}(意识|昏迷|呕吐|视力)", "剧烈头痛伴意识/神经症状"),
    (r"(意识模糊|意识丧失|昏迷|抽搐)", "意识障碍"),
    (r"胸痛.{0,50}(呼吸困难|气短|大汗|放射)", "胸痛伴心血管症状"),
    (r"((肢体|腿|手臂|上肢|下肢)?突发无力|突然麻木|半身不遂|口角歪斜)", "疑似卒中"),
    (r"(外伤|摔倒|摔伤).{0,50}(骨折|畸形|变形|开放性|出血不止)", "严重外伤/疑似骨折"),
    (r"(马尾综合征|鞍区麻木|大小便失禁|括约肌)", "马尾综合征"),
    (r"脊髓.{0,50}(损伤|压迫|截瘫|四肢瘫)", "脊髓损伤"),
    (r"(高处坠落|车祸|重物砸伤).{0,50}(脊柱|颈椎|腰椎)", "高能量外伤"),
    (r"(呼吸困难|窒息|无法呼吸)", "呼吸困难"),
    (r"(大量出血|止不住血|动脉出血)", "大出血"),
    (r"关节.{0,50}(脱位|弹响后无法活动|锁死)", "关节脱位/交锁"),
...
HIGH_RISK_PATTERNS: list[tuple[str, str, str]] = [
    # (正则模式, 规则描述, 建议科室)
    (r"(VAS|NRS|疼痛评分).{0,50}([89]|10)分?.{0,30}(持续|超过)", "剧烈疼痛持续", "疼痛科/骨科"),
    (r"夜间痛.{0,50}(影响睡眠|无法入睡).{0,30}(超过|两|2).{0,5}周", "夜间痛>2周", "骨科/肿瘤科"),
    (r"(不明原因|无外伤).{0,30}体重(下降|减轻)", "不明原因体重下降", "骨科/肿瘤科"),
    (r"发(热|烧).{0,50}(关节|脊柱|骨).{0,30}(疼痛|肿)", "发热伴骨关节痛", "骨科/感染科"),
    (r"疼痛.{0,30}(进行性|逐渐|越来越).{0,10}加重", "进行性疼痛加重", "骨科"),
    (r"(下肢|腿|脚).{0,30}(放射痛|串痛|麻木).{0,30}(加重|无力)", "神经根症状加重", "脊柱科/神经外科"),
    (r"(外伤|摔倒).{0,30}(48|72|两天|三天).{0,30}(肿胀加重|不能活动)", "外伤后持续加重", "急诊/骨科"),
    (r"(长期|超过3个月|慢性).{0,30}(服用|吃).{0,30}(止痛药|布洛芬|芬必得)", "长期NSAID使用", "疼痛科"),
    (r"(关节红肿热痛|痛风发作|急性痛风)", "急性关节炎", "风湿免疫科/骨科"),
    (r"(骨质疏松|T值).{0,30}(-2\.5|严重)", "严重骨质疏松", "骨科/内分泌科"),
]

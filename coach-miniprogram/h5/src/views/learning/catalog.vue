...
function openContent(item: any) {
  if (item.type === 'video') uni.navigateTo({ url: '/learning/video-player?id=' + item.id })
  else if (item.type === 'audio') uni.navigateTo({ url: '/learning/audio-player?id=' + item.id })
  else uni.navigateTo({ url: '/learning/content-detail?id=' + item.id })
}

  ...
  <view class="cf-page">
    ...
    <scroll-view scroll-y class="cf-body">
      ...
      <template v-else-if="queue.length">
        <view v-for="item in queue" :key="item.id" class="cf-card" :class="{ 'cf-card--done': item._handled }">
    ...
    <view class="cf-modal-mask" v-if="agentResult" @tap="agentResult = null">
      <view class="cf-modal cf-modal--result" @tap.stop>
        ...
        <view class="cf-result-list">
          ...
          <view v-if="!(agentResult.suggestions || []).length" class="cf-empty-inline"><text>暂无建议</text></view>
        </view>
        <view class="cf-modal__actions">
          <view class="cf-modal__btn cf-modal__btn--confirm cf-modal__btn--full" @tap="agentResult = null"><text>关闭</text></view>
        </view>
      </view>
    </view>
    ...
    <view class="cf-modal-mask" v-if="rejectTarget" @tap="rejectTarget = null">
      <view class="cf-modal" @tap.stop>
        ...
        <textarea class="cf-modal__input" v-model="rejectReason" placeholder="请输入退回原因..." :maxlength="200" />
        <view class="cf-modal__actions">
...
// ============================================================
// 飞轮页 API（内联，绕过分包 require 主包限制）
// ============================================================
const api = {
  getQueue: (p?: Record<string, any>) => _get<{ items: any[] }>('/v1/coach/review-queue', p),
  getStats: () => _get<any>('/v1/coach/stats/today'),
  approve:  (id: string, d?: Record<string, any>) => _post<any>(`/v1/coach/review/${id}/approve`, d || {}),
  reject:   (id: string, reason: string) => _post<any>(`/v1/coach/review/${id}/reject`, { reason }),
  dashboard: () => _get<any>('/v1/coach/dashboard'),
  runAgent: (sid: number, input?: string) => _post<any>('/v1/agent/run', {
    agent_type: 'COACHING', user_id: String(sid), input: input || '为学员生成个性化跟进计划',
  }),
}
...
const studentNames = computed(() => studentList.value.map(s => s.name || s.full_name || s.username))

onMounted(() => { loadQueue(); loadStats(); loadStudentList() })

async function loadQueue() {
  loading.value = true
  try {
    const res = await api.getQueue()
    queue.value = (res.items || []).map((item: any) => ({ ...item, _handled: false, _action: '', _expanded: false }))
  } catch { queue.value = [] }
...
async function loadStats() {
  try {
    ...
    stats.value = { pending: res.pending ?? 0, total_reviewed: res.total_reviewed ?? 0, approved: res.approved ?? 0, rejected: res.rejected ?? 0 }
  } catch { /* keep defaults */ }
}

function toggleDraft(item: any) { item._expanded = !item._expanded }

async function handleApprove(item: any) {
  try {
    await api.approve(item.id)
    item._handled = true; item._action = 'approved'
    uni.showToast({ title: '审核通过', icon: 'success' }); loadStats()
  } catch (e: any) { uni.showToast({ title: e?.message || '操作失败', icon: 'none' }) }
}

function showRejectModal(item: any) { rejectTarget.value = item; rejectReason.value = '' }
...
async function confirmReject() {
  ...
  try {
    ...
    item._handled = true; item._action = 'rejected'; rejectTarget.value = null
    uni.showToast({ title: '已退回', icon: 'none' }); loadStats()
  } catch (e: any) { uni.showToast({ title: e?.message || '操作失败', icon: 'none' }) }
}
...
async function loadStudentList() {
  try {
    ...
    studentList.value = (res.students || []).map((s: any) => ({ ...s, name: s.name || s.full_name || s.username }))
...
async function runFollowup() {
  ...
  try {
    const res = await api.runAgent(pickedStudent.value.id, '生成个性化跟进计划')
...
.cf-stats { display: flex; background: var(--surface); padding: 24rpx 32rpx; border-bottom: 1px solid var(--border-light); gap: 8rpx; }
.cf-stat { flex: 1; text-align: center; }
.cf-stat__val { display: block; font-size: 40rpx; font-weight: 800; }
.cf-stat__val--orange { color: #f59e0b; } .cf-stat__val--blue { color: #3b82f6; } .cf-stat__val--green { color: #10b981; } .cf-stat__val--red { color: #ef4444; }
...
.cf-card { position: relative; background: var(--surface); border-radius: var(--radius-lg); padding: 24rpx; margin-bottom: 20rpx; border: 1px solid var(--border-light); overflow: hidden; transition: opacity 0.3s; }
.cf-card--done { opacity: 0.55; }
.cf-card__done-overlay { position: absolute; top: 16rpx; right: 16rpx; z-index: 2; }
.cf-card__done-text { font-size: 24rpx; font-weight: 700; padding: 4rpx 16rpx; border-radius: var(--radius-full); }
.cf-card__done-text--green { background: #f0fdf4; color: #16a34a; } .cf-card__done-text--red { background: #fef2f2; color: #dc2626; }
...
.cf-card__type--assessment { background: #faf5ff; color: #7c3aed; } .cf-card__type--ai_reply { background: #f0fdf4; color: #16a34a; } .cf-card__type--push { background: #fffbeb; color: #d97706; }
...
.cf-card__draft-text { display: block; font-size: 26rpx; color: var(--text-primary); line-height: 1.6; white-space: pre-wrap; word-break: break-all; }
...
.cf-card__rx-key { font-size: 22rpx; color: var(--text-secondary); width: 160rpx; flex-shrink: 0; } .cf-card__rx-val { font-size: 22rpx; color: var(--text-primary); flex: 1; }
.cf-card__actions { display: flex; gap: 16rpx; margin-top: 16rpx; }
.cf-btn { flex: 1; height: 72rpx; border-radius: var(--radius-lg); display: flex; align-items: center; justify-content: center; font-size: 26rpx; font-weight: 700; }
.cf-btn:active { opacity: 0.8; }
.cf-btn--approve { background: #10b981; color: #fff; } .cf-btn--reject { background: #fef2f2; color: #dc2626; border: 1px solid #fecaca; }
.cf-card__wait { display: block; font-size: 20rpx; color: var(--text-tertiary); margin-top: 8rpx; text-align: right; }
.cf-empty { display: flex; flex-direction: column; align-items: center; padding: 160rpx 0; gap: 20rpx; }
.cf-empty__icon { width: 120rpx; height: 120rpx; border-radius: 50%; background: #f0fdf4; color: #10b981; display: flex; align-items: center; justify-content: center; font-size: 56rpx; font-weight: 700; }
...
.cf-modal { width: 85%; background: var(--surface); border-radius: var(--radius-xl); padding: 32rpx; }
.cf-modal__title { display: block; font-size: 30rpx; font-weight: 700; color: var(--text-primary); margin-bottom: 24rpx; }
.cf-modal__input { width: 100%; height: 180rpx; padding: 16rpx 20rpx; background: var(--surface-secondary); border-radius: var(--radius-lg); border: 1px solid var(--border-light); font-size: 26rpx; color: var(--text-primary); box-sizing: border-box; }
...
.cf-modal__btn--cancel { background: var(--surface-secondary); color: var(--text-secondary); } .cf-modal__btn--ok { background: #ef4444; color: #fff; } .cf-modal__btn--confirm { background: #10b981; color: #fff; } .cf-modal__btn--full { flex: 1; }
...
.cf-picker-trigger { display: flex; justify-content: space-between; align-items: center; padding: 20rpx 24rpx; background: var(--surface-secondary); border-radius: var(--radius-lg); border: 1px solid var(--border-light); font-size: 28rpx; color: var(--text-primary); margin-bottom: 24rpx; }
...
.cf-result-confidence__label { font-size: 24rpx; color: var(--text-secondary); } .cf-result-confidence__val { font-size: 32rpx; font-weight: 800; color: #10b981; }
...
.cf-result-item__idx { width: 40rpx; height: 40rpx; border-radius: 50%; flex-shrink: 0; background: var(--bhp-primary-500); color: #fff; display: flex; align-items: center; justify-content: center; font-size: 22rpx; font-weight: 700; }
.cf-result-item__text { flex: 1; font-size: 26rpx; color: var(--text-primary); line-height: 1.5; word-break: break-all; white-space: normal; }

...
revision = 'v41_week2_001'
down_revision = None  # ← 填入当前最新 revision ID
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    for table in COACH_TABLES:
        ...
        # 给view添加INSERT/UPDATE/DELETE规则（让ORM写操作也能透传）
        op.execute(f"""
            CREATE OR REPLACE RULE "{table}_insert" AS
            ON INSERT TO public.{table}
            DO INSTEAD INSERT INTO {SCHEMA}.{table} VALUES (NEW.*);
        """)
        op.execute(f"""
            CREATE OR REPLACE RULE "{table}_update" AS
            ON UPDATE TO public.{table}
            DO INSTEAD UPDATE {SCHEMA}.{table} SET *  = NEW.* WHERE {SCHEMA}.{table}.id = OLD.id;
        """)
        op.execute(f"""
            CREATE OR REPLACE RULE "{table}_delete" AS
            ON DELETE TO public.{table}
            DO INSTEAD DELETE FROM {SCHEMA}.{table} WHERE {SCHEMA}.{table}.id = OLD.id;
        """)
...
def downgrade() -> None:
    ...
    for table in COACH_TABLES:
        # 1. 删除兼容view的规则
        for rule_type in ['insert', 'update', 'delete']:
            op.execute(f'DROP RULE IF EXISTS "{table}_{rule_type}" ON public.{table}')
